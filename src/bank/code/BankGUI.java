...
public class	BankGUI
	...
	public 	BankGUI (java.util.List<Bank> banken)
		...
		if (getBanken ().size () > 0) 
			...
			if (getBank ().numKonto () > 0) setKonto (getBank ().getKonto (0));
			}
			...
			(
				new ActionListener ()
					...
					public void	actionPerformed (ActionEvent ev)
						...
						if (getBanken ().size () == 0) setBank (null);
						}
					}
			);
		}
	...
	public void	machBankAuswahl ()
		...
		c.addItemListener (new ItemListener ()
			...
			public void	itemStateChanged (ItemEvent ev)
				...
				((CardLayout) getKarten ().getLayout ())
					.show (getKarten (), "LeereKarte");
				}
			});
		}
	...
	private void	zeigKonten (JPanel p)
		...
		if (getBank () != null) 
			...
			for (Konto	k : getBank ().getKonto ())
				{
				c.gridx = 1; ++c.gridy; c.anchor = GridBagConstraints.WEST;
				c.gridwidth = 2;
	...
	public void	machKarteEinAuszahlung ()
		...
		c.gridx = 2; c.gridy = 3; c.fill = GridBagConstraints.BOTH;
		karte.add (t2, c);
			...
			public void	actionPerformed (ActionEvent ev)
				...
				if ((s = t.getText ().trim ()).length () != 0)
					...
					try
						...
						//System.err.println ("Eingezahlt (" + s + ") " + getKonto ().getEuro ());
						}
				...
				if ((s = t2.getText ().trim ()).length () != 0)
					...
					try
						...
						//System.err.println ("Ausgezahlt (" + s + ") " + getKonto ().getEuro ());
						}
						catch (KontoFehler e)
							{
	...
	public void	machKarteBankAnlegen ()
		...
		c.gridx = 1; c. gridy = 1; c.gridheight = 3;
		karte.add (p, c);
		
		c.gridx = 1; c. gridy = 4; c.gridheight = 1;
		karte.add (new JLabel ("---"), c);
		++c.gridy;
	...
	public void	machKarteKontoAnlegen ()
		...
		c.gridx = 2; c.gridy = 4; c.fill = GridBagConstraints.BOTH;
		karte4.add (t43, c);
	...
	public void	machKarteUeberweisung ()
		...
		jb.addActionListener (new ActionListener ()
			...
			public void	actionPerformed (ActionEvent ev)
				...
				else if ((s = getUeberweisungBetrag ().getText ().trim ()).length () != 0)
					{
					try
						{
						getVonKonto ().auszahlen (s);
						getNachKonto ().einzahlen (s);
						//System.err.println ("Ausgezahlt (" + s + ") " + getVonKonto ().getEuro ());
						//System.err.println ("Eingezahlt (" + s + ") " + getNachKonto ().getEuro ());
						}
						catch (Exception e)
							{
				...
				if (falsch.length () > 0)
					...
					getUeberweisungMeldung ().setText ("Überweisung wurde getätigt.");
					}
				((CardLayout) getKarten ().getLayout ())
					.show (getKarten (), "Überweisung");
				}
		...
		karte.add (new JLabel ("Konto"), c);
		c.gridx = 2; c.gridy = 9; c.gridwidth = 2;
		karte.add (getKontoNachAuswahl (), c);
		
		c.gridx = 1; c.gridy = 10; c.gridwidth = 1;
		karte.add (new JLabel (" "), c);
		c.gridx = 1; c.gridy = 11; c.gridwidth = 3;
		karte.add (jb, c);
